...
/* cjc */
uint8_t Adafruit_Fingerprint::ReadIndexTable(void)
{
  GET_CMD_PACKET(PS_ReadIndexTable, 0x0);
  uint8_t result = 0;
  // Serial.print("\n**packet.data[]:");

  for (int i = 1; i < 33; i++)
  {
    // ESP_LOGE(TAG, "packet.data[%d]:%d", i, packet.data[i]);
    // // Serial.print(packet.data[i], DEC);
    // Serial.print(packet.data[i], BIN);
    // Serial.println("");

    if (packet.data[i] == 0x0)
    {
      if (i == 1)
      {
        result = 1;
    ...
    else
    {
      for (int j = 0; j < 8; j++)
      {
        // ESP_LOGE(TAG, "packet.data[%d] %d位置 结果%d:", i, j, packet.data[i] & (1 << j));
        if (i == 1 && j == 0)
        {
          result++;
          continue;
        }
        if (packet.data[i] & (1 << j))
        {
          result++;
        }
        else
...
/* cjc */
uint8_t Adafruit_Fingerprint::send_fpdata(uint8_t slot, uint8_t *finger_code)
{
  GET_CMD_PACKET(0x09, 1);
  // Serial.println(finger_code);
  delay(10);
  Adafruit_Fingerprint_Packet packet2(FINGERPRINT_DATAPACKET, 128, finger_code);
  Adafruit_Fingerprint_Packet packet3(FINGERPRINT_DATAPACKET, 128, finger_code + 128);
  Adafruit_Fingerprint_Packet packet4(FINGERPRINT_DATAPACKET, 128, finger_code + 256);
  Adafruit_Fingerprint_Packet packet5(FINGERPRINT_ENDDATAPACKET, 128, finger_code + 384);
  mySerial->flush();
  writeStructuredPacket(packet2);
  writeStructuredPacket(packet3);
  writeStructuredPacket(packet4);
  writeStructuredPacket(packet5);
  // for(int i=0;i<556;i++){
  // mySerial->write(finger_code[i]);
  // }
  return storeModel(slot);
  // return 1;
}

/* cjc */
uint8_t Adafruit_Fingerprint::auto_enroll(uint16_t location, uint8_t count, uint16_t parm)
{
  SEND_CMD_PACKET(AUtO_ENROLL, (uint8_t)(location >> 8), (uint8_t)(location & 0xFF), count, (uint8_t)(parm >> 8), (uint8_t)(parm & 0xFF));
  return 0;
}

uint8_t Adafruit_Fingerprint::auto_enroll_replay(uint8_t *reply)
{
  Adafruit_Fingerprint_Packet packet(0x07, 14, reply);
  if (!getStructuredPacket(&packet, 0))
  {
    memcpy(reply, packet.data, 5);
  }
...
/* cjc */
uint8_t Adafruit_Fingerprint::PS_Match()
{
  GET_CMD_PACKET(0x03);
  fingerID = 0xFFFF;
  confidence = 0xFFFF;
