...
/* cjc */
#define AUtO_ENROLL 0x31 //!< Search for fingerprint in slot
...
#define PS_ReadIndexTable 0x1F         //!< Read finger template numbers
// #define FINGERPRINT_AURALEDCONFIG 0x35 //!< Aura LED control
#define FINGERPRINT_AURALEDCONFIG 0x3c //!< Aura LED control
#define FINGERPRINT_LEDON 0x04         //!< Turn on the onboard LED
#define FINGERPRINT_LEDOFF 0x03        //!< Turn off the onboard LED
...
public:
  ...
  /* cjc */
  uint8_t send_fpdata(uint8_t slot, uint8_t *finger_code);
  uint8_t auto_enroll(uint16_t id, uint8_t count, uint16_t parm);
  uint8_t auto_enroll_replay(uint8_t *reply);
